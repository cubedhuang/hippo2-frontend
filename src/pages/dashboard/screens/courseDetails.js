...
function DashboardCourseDetails({ setIsStudentRegistered }) {
	...
	useEffect(() => {
		(async () => {
			...
			const scholarshipData = data.data.filter((order) => order.scholarship !== undefined);
			...
			coursesList.push((
				<div key={course.order.id} className="container flex flex-wrap mx-auto mt-12 px-6 pb-6">
					<div className={`flex-none md:flex-initial w-full md:w-7/12 py-8 px-16 pb-10 text-lg text-black bg-white rounded-t-xl ${isFullScholarship ? 'rounded-t-xl md:rounded-l-xl md:rounded-none' : 'md:rounded-tl-xl md:rounded-none'}`}>
						...
						<p className="mb-5"><a href={`/orders/${course.order.id}/change-batch`} className="text-blue-500 underline">Want to change your batch?</a><span className="italic text-red-400"> (Deadline: {course.course.batch_change_deadline})</span></p>
						{scholarship}
						<Link to={`/orders/${course.order.id}/refund`} className='mb-4'>
				...
				for (const scholarshipOrder of scholarships) {
					if (scholarshipOrder.scholarship.status.startsWith('ACCEPTED_')) {
						...
						scholarshipList.push((
							<div className="container flex flex-wrap mx-auto mb-10 px-6 pb-6" key={scholarshipOrder.order.id}>
						...
						scholarshipList.push((
							<div className="container flex flex-wrap mx-auto mb-10 px-6 pb-6 mb-8" key={scholarshipOrder.order.id}>
